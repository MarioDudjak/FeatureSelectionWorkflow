...
def feature_subset_aggregation(dataset, run, population, population_fitness,
                               search_results,
                               best_test_fitness):
    fitness_function = ClassificationProblem(file, classifier,
                                                random_state1=42,  # dodati run + za variable
                                                random_state2=42,  # dodati run + za variable
    ...
    fitness_function_CV2 = ClassificationProblemCV(file, classifier,
                                                   random_state1=42,
                                                   random_state2=5,  # dodati run + za variable
                                                   test_size=experiment_setup["test_size"],
                                                   k_folds=5,
                                                   wrapper_fitness_metric='f1_macro',
                                                   metrics=classification_metrics)
    ...
    results = {"dataset": dataset, "run": run, "best_solution": best_solution, "best_solution_size": best_solution_size,
               ...
               "best_solution_test_sensitivity": test_sensitivity}
    ...
    if aggregation_principles_to_evaluate['sbs_union']:
        ...
        union_fitness = fitness_function_CV2.evaluate_on_validation(union)
        ...
        for j in activated_features_sorted_indices:
            if activated_features[j] != 0 and activated_features[j] != 1:
                ...
                sbs_fitness = fitness_function_CV2.evaluate_on_validation(sbs_union)
    ...
    if aggregation_principles_to_evaluate['sfs_intersection']:
        ...
        intersection_fitness = fitness_function_CV2.evaluate_on_validation(intersection)
        ...
        for j in activated_features_sorted_indices:
            if activated_features[j] != 0 and activated_features[j] != 1:
                ...
                sfs_fitness = fitness_function_CV2.evaluate_on_validation(sfs_intersection)
    ...
    if aggregation_principles_to_evaluate['sfs_sbs_majority_voting']:
        ...
        majority_voting_fitness = fitness_function_CV2.evaluate_on_validation(majority_voting)
        ...
        for j in activated_features_sorted_indices:
            if feature_thresholds[j] != 0.5:
                ...
                if activated_features[j] < 0.5:
                    ...
                    sbs_sfs_majority_voting_fitness = fitness_function_CV2.evaluate_on_validation(
    ...
    #         union = union + np.array(solution).astype(bool)
    #
    #     coalition_union_fitness = fitness_function_CV2.evaluate_on_validation(union)
    ...
    #             old_fitness = sbs_fitness
    #             sbs_fitness = fitness_function_CV2.evaluate_on_validation(coalition_sbs_union)
    #
    #             if sbs_fitness < old_fitness:
    ...
    #     results["coalition_sbs_union_kfold_fitness"] = fitness_function_CV.evaluate_on_validation(coalition_sbs_union)
    #
    # if aggregation_principles_to_evaluate['sfs_intersection']:
    #     activated_features = np.sum(coalition, axis=0) / len(coalition)
    #     activated_features_sorted_indices = np.argsort(activated_features)[::-1]
    ...
    #             old_fitness = sfs_fitness
    #             sfs_fitness = fitness_function_CV2.evaluate_on_validation(coalition_sfs_intersection)
    #
    #             if sfs_fitness <= old_fitness:
    ...
    #     results["coalition_sfs_intersection_kfold_fitness"] = fitness_function_CV.evaluate_on_validation(
    #         coalition_sfs_intersection)
    #
    # if aggregation_principles_to_evaluate['sfs_sbs_majority_voting']:
    #     activated_features = np.sum(coalition, axis=0) / len(coalition)
    ...
    #     activated_features_sorted_indices = np.argsort(feature_thresholds)
    #
    #     coalition_sbs_sfs_majority_voting_fitness = fitness_function_CV2.evaluate_on_validation(
    ...
    #     results[
    #         "coalition_sbs_sfs_majority_voting_kfold_fitness"] = fitness_function_CV.evaluate_on_validation(
    #         coalition_sbs_sfs_majority_voting)
    #
...
for optimiser in fs_wrappers:
    for classifier in classifiers:
        summary_results[optimiser.name + classifier.name] = []
        asm_stability_results[optimiser.name + classifier.name] = []
        anhd_stability_results[optimiser.name + classifier.name] = []
        for file in datasets:
            ...
            population_quality_filename = '_'.join(
                ['', optimiser.name, classifier.name, file.name])
                    ...
                    if idx % 14 == 13:  # 13 jer se gleda best solution na kraju runa
                        search_results.append(
                            [float(data[0]), float(test_data[idx // 14][0]), float(test_data[idx // 14][1]),
                             test_data[idx // 14][-3]])  # 0 je f1:macro, 1 CAC

            filename = '_'.join(
                ['',optimiser.name + "_", classifier.name, file.name,
                 'archive'])
            ...
            if test_header is not None and test_data is not None:
                ...
                for idx, candidate in enumerate(test_data):
                    if idx > 0 and idx % population_size == 0:
                        summary_results[optimiser.name + classifier.name].append(
                ...
                asm_stability_results[optimiser.name + classifier.name].append(
                    get_stability_report(
                        summary_results[optimiser.name + classifier.name], file.name, calculate_ASM))
        ...
        for row in summary_results[optimiser.name + classifier.name]:
            CsvProcessor().save_summary_results(filename='_'.join(
                [optimiser.name, classifier.name, "_archive_combinations"]),
                header=list(row.keys()),
                data=list(row.values()))

        for row in asm_stability_results[optimiser.name + classifier.name]:
            CsvProcessor().save_summary_results(filename='_'.join(
                [optimiser.name, classifier.name, "_archive_asm_stabilities"]),
                header=list(row.keys()),
                data=list(row.values()))
