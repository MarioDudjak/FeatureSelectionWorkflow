...
aggregation_principles_to_evaluate = {
    ...
    'majority_overvoting': 0,
    'complement': 0,
    'fitness_weighted_majority_overvoting': 1
}
...
def feature_subset_aggregation(dataset, X, y, run, population, worst_population, population_fitness, population_fitness_worst, population_kfold_fitness,
    ...
    if aggregation_principles_to_evaluate['complement']:
        worst_activated_features = np.sum(worst_population, axis=0)
        activated_features = worst_activated_features < (len(population) // 2)
        complement = activated_features * 1

        fitness = fitness_function.evaluate_on_validation(complement)
        test_CAC = fitness_function_CAC.evaluate_on_test(complement)
        test_F1 = fitness_function_F1.evaluate_on_test(complement)
        complement_db_index = get_Davies_Bouldin_score(X, y, complement)
        complement_silhouette_index = get_silhouette_score(X, y, complement)

        complement = complement * 1
        results["complement"] = ','.join([str(feature) for feature in complement])
        results["complement_validation_fitness"] = fitness
        results["complement_test_CAC"] = test_CAC
        results["complement_test_F1"] = test_F1
        results["complement_size"] = np.sum(complement)
        results["complement_DB"] = complement_db_index
        results["complement_silhouette"] = complement_silhouette_index
        val_sensitivity = fitness_function_sensitivity.evaluate_on_validation(complement)
        kfold_fitness = fitness_function_CV.evaluate_on_validation(complement)
        results["complement_val_sensitivity"] = val_sensitivity
        results["complement_kfold_fitness"] = kfold_fitness

    if aggregation_principles_to_evaluate['fitness_weighted_majority_overvoting']:
        best_activated_features = np.zeros(len(population[0]), dtype=float)
        worst_activated_features = np.zeros(len(population[0]), dtype=float)
        total_fitness = 0
        for idx, good_candidate in enumerate(population):
            best_activated_features += population_fitness[idx] * np.array(good_candidate).astype(int)
            total_fitness += population_fitness[idx]

        for idx, bad_candidate in enumerate(worst_population):
            worst_activated_features += population_fitness_worst[idx] * np.array(bad_candidate).astype(int)

        activated_features = best_activated_features - worst_activated_features
        fitness_weighted_majority_overvoting = (activated_features >= (total_fitness // 2)) * 1

        fitness = fitness_function.evaluate_on_validation(fitness_weighted_majority_overvoting)
        test_CAC = fitness_function_CAC.evaluate_on_test(fitness_weighted_majority_overvoting)
        test_F1 = fitness_function_F1.evaluate_on_test(fitness_weighted_majority_overvoting)
        fitness_weighted_majority_overvoting_db_index = get_Davies_Bouldin_score(X, y, fitness_weighted_majority_overvoting)
        fitness_weighted_majority_overvoting_silhouette_index = get_silhouette_score(X, y, fitness_weighted_majority_overvoting)

        fitness_weighted_majority_overvoting = fitness_weighted_majority_overvoting * 1
        results["fitness_weighted_majority_overvoting"] = ','.join([str(feature) for feature in fitness_weighted_majority_overvoting])
        results["fitness_weighted_majority_overvoting_validation_fitness"] = fitness
        results["fitness_weighted_majority_overvoting_test_CAC"] = test_CAC
        results["fitness_weighted_majority_overvoting_test_F1"] = test_F1
        results["fitness_weighted_majority_overvoting_size"] = np.sum(fitness_weighted_majority_overvoting)
        results["fitness_weighted_majority_overvoting_DB"] = fitness_weighted_majority_overvoting_db_index
        results["fitness_weighted_majority_overvoting_silhouette"] = fitness_weighted_majority_overvoting_silhouette_index
        val_sensitivity = fitness_function_sensitivity.evaluate_on_validation(fitness_weighted_majority_overvoting)
        kfold_fitness = fitness_function_CV.evaluate_on_validation(fitness_weighted_majority_overvoting)
        results["fitness_weighted_majority_overvoting_val_sensitivity"] = val_sensitivity
        results["fitness_weighted_majority_overvotingkfold_fitness"] = kfold_fitness
    ...
    for procedure in evaluation_procedures:
        for classifier in classifiers:
            ...
            for file in datasets:
                ...
                worst_test_header, worst_test_data = CsvProcessor().read_file(
                    filename='logs/archive/' + filename + "_test_worst")

                if test_header is not None and test_data is not None:
                    test_data = [row for row in test_data if row]
                    test_data = np.array(test_data)

                if worst_test_header is not None and worst_test_data is not None:
                    worst_test_data = [row for row in worst_test_data if row]
                    worst_test_data = np.array(worst_test_data)

                if test_header is not None and test_data is not None:
                    run = 0
                    index = 0
                    population = np.empty([population_size, dimensionality], dtype=bool)
                    worst_population = np.empty([population_size, dimensionality], dtype=bool)
                    population_fitness = np.zeros(population_size)
                    population_fitness_worst = np.zeros(population_size)
                    ...
                    fitness_function_CV = ClassificationProblemCV(file, classifier,
                                                                  ...
                                                                  wrapper_fitness_metric=fitness,
                                                                  metrics=classification_metrics)
                    for idx, (candidate, worst_candidate) in enumerate(zip(test_data, worst_test_data)):
                        if idx > 0 and idx % population_size == 0:
                            summary_results[optimiser.name + procedure + classifier.name].append(
                                feature_subset_aggregation(file.name, X_rest, y_rest, run, copy.deepcopy(population),
                                                           copy.deepcopy(worst_population),
                                                           copy.deepcopy(population_fitness),
                                                           population_fitness_worst,
                        ...
                        worst_population[index] = [True if bit == '1' else False for bit in
                                                   worst_candidate[header_idx['vector_val'] -1]]
                        ...
                        population_fitness_worst[index] = float(worst_candidate[header_idx[fitness + "_val"]])
            ...
            for row in summary_results[optimiser.name + procedure + classifier.name]:
                CsvProcessor().save_summary_results(filename='_'.join(
                    [procedure, optimiser.name, classifier.name, "_archive_combinations_best_worst"]),
                    header=list(row.keys()),
                    data=list(row.values()))

            for row in asm_stability_results[optimiser.name + procedure + classifier.name]:
                CsvProcessor().save_summary_results(filename='_'.join(
                    [procedure, optimiser.name, classifier.name, "_archive_asm_stabilities_best_worst"]),
                    header=list(row.keys()),
                    data=list(row.values()))

            # for row in anhd_stability_results[optimiser.name + procedure + classifier.name]:
            #     CsvProcessor().save_summary_results(filename='_'.join(
            #         [procedure, optimiser.name, classifier.name, "_archive_anhd_stabilities_best_worst"]),
            #         header=list(row.keys()),
            #         data=list(row.values()))
