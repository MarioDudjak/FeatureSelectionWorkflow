...
aggregation_principles_to_evaluate = {
    ...
    'filter_scores_voting_solutions': 0,
    'sbs_union': 1,
    'sfs_intersection': 1,
    'sfs_sbs_majority_voting': 1
}
    ...
    for i in range(len(population) - 1):
        for j in range(i + 1, len(population)):
            if (sum(population[i]) == 0) or (sum(population[j]) == 0):
                continue
            elif (sum(population[i]) == 0 and sum(population[j]) == 0) or (
...
def calculate_chi2(X, y):
    ...
    fs.fit(X, y)
    normalised_feature_scores = [0.1 + 0.8 * (score - np.min(fs.scores_)) / (np.max(fs.scores_) - np.min(fs.scores_))
...
def calculate_ReliefF(X, y):
    ...
    normalised_feature_scores = [
        0.1 + 0.8 * (score - np.min(feature_scores)) / (np.max(feature_scores) - np.min(feature_scores)) for score in
        feature_scores]
    return normalised_feature_scores
...
def feature_subset_aggregation(dataset, X, y, run, population, population_fitness, population_kfold_fitness,
    ...
    fitness_function_sensitivity = ClassificationProblem(file, classifier,
                                                         ...
                                                         validation_size=experiment_setup[
                                                             "validation_size"],
                                                         wrapper_fitness_metric='recall',
                                                         metrics=classification_metrics)
    fitness_function_CV = ClassificationProblemCV(file, classifier,
                                                  random_state=5,  # dodati run + za variable
                                                  test_size=experiment_setup["test_size"],
                                                  k_folds=5,
                                                  wrapper_fitness_metric='f1_macro',
                                                  metrics=classification_metrics)
    ...
    test_sensitivity = fitness_function_sensitivity.evaluate_on_test(best_solution)
    ...
    results = {"dataset": dataset, "run": run, "best_solution": best_solution, "best_solution_size": best_solution_size,
               ...
               "best_solution_test_sensitivity": test_sensitivity, "best_solution_kfold_fitness": kfold_fitness}
    ...
    if aggregation_principles_to_evaluate['majority_voting']:
        ...
        test_sensitivity = fitness_function_sensitivity.evaluate_on_test(majority_voting)
        kfold_fitness = fitness_function_CV.evaluate_on_validation(majority_voting)
        results["majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['minority_voting']:
        ...
        test_sensitivity = fitness_function_sensitivity.evaluate_on_test(minority_voting)
        kfold_fitness = fitness_function_CV.evaluate_on_validation(minority_voting)
        results["minority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['large_majority_voting']:
        ...
        test_sensitivity = fitness_function_sensitivity.evaluate_on_test(large_majority_voting)
        kfold_fitness = fitness_function_CV.evaluate_on_validation(large_majority_voting)
        results["large_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['fitness_weighted_majority_voting']:
        ...
        test_sensitivity = fitness_function_sensitivity.evaluate_on_test(fitness_weighted_majority_voting)
        kfold_fitness = fitness_function_CV.evaluate_on_validation(fitness_weighted_majority_voting)
        results["fitness_weighted_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['fitness_rank_weighted_majority_voting']:
        ...
        total_votes = 0
        for idx, candidate in enumerate(population):
            rank = population_fitness[population_fitness <= population_fitness[idx]].shape[0]
        ...
        test_sensitivity = fitness_function_sensitivity.evaluate_on_test(fitness_rank_weighted_majority_voting)
        kfold_fitness = fitness_function_CV.evaluate_on_validation(fitness_rank_weighted_majority_voting)
        results["fitness_rank_weighted_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['size_weighted_majority_voting']:
        ...
        test_sensitivity = fitness_function_sensitivity.evaluate_on_test(size_weighted_majority_voting)
        kfold_fitness = fitness_function_CV.evaluate_on_validation(size_weighted_majority_voting)
        results["size_weighted_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['fitness_size_rank_weighted_majority_voting_solutions']:
        ...
        test_sensitivity = fitness_function_sensitivity.evaluate_on_test(
            fitness_size_rank_weighted_majority_voting)
        kfold_fitness = fitness_function_CV.evaluate_on_validation(fitness_size_rank_weighted_majority_voting)
        results["fitness_size_rank_weighted_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['fitness_size_weighted_majority_voting_solutions']:
        ...
        total_votes = 0
        for idx, candidate in enumerate(population):
            weight = population_fitness[idx] + ((len(candidate) - np.sum(candidate)) / len(candidate))
        ...
        test_sensitivity = fitness_function_sensitivity.evaluate_on_test(fitness_size_weighted_majority_voting)
        kfold_fitness = fitness_function_CV.evaluate_on_validation(fitness_size_weighted_majority_voting)
        results["fitness_size_weighted_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['db_rank_weighted_majority_voting_solutions']:
        ...
        test_sensitivity = fitness_function_sensitivity.evaluate_on_test(db_rank_weighted_majority_voting)
        kfold_fitness = fitness_function_CV.evaluate_on_validation(db_rank_weighted_majority_voting)
        results["db_rank_weighted_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['db_weighted_majority_voting_solutions']:
        ...
        test_sensitivity = fitness_function_sensitivity.evaluate_on_test(db_weighted_majority_voting)
        kfold_fitness = fitness_function_CV.evaluate_on_validation(db_weighted_majority_voting)
        results["db_weighted_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['pairwise_feature_frequency_majority_voting_solutions']:
        ...
        results["pairwise_feature_frequency_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['filter_scores_voting_solutions']:
        ...
        test_sensitivity = fitness_function_sensitivity.evaluate_on_test(filter_scores_majority_voting)
        kfold_fitness = fitness_function_CV.evaluate_on_validation(filter_scores_majority_voting)
        results["filter_scores_majority_voting_test_sensitivity"] = test_sensitivity
        results["filter_scores_majority_voting_kfold_fitness"] = kfold_fitness

    if aggregation_principles_to_evaluate['sbs_union']:
        activated_features = np.sum(population, axis=0) / len(population)
        activated_features_sorted_indices = np.argsort(activated_features)
        for solution in population:
            union = union + np.array(solution).astype(bool)

        union_fitness = fitness_function_CV.evaluate_on_validation(union)
        sbs_union = copy.deepcopy(union)
        sbs_fitness = union_fitness
        for j in activated_features_sorted_indices:
            if activated_features[j] != 0 and activated_features[j] != 1:
                sbs_union[j] = False
                old_fitness = sbs_fitness
                sbs_fitness = fitness_function_CV.evaluate_on_validation(sbs_union)

                if sbs_fitness < old_fitness:
                    sbs_union[j] = True
                    sbs_fitness = old_fitness
                    # break

        sbs_union_validation_fitness = fitness_function.evaluate_on_validation(sbs_union)
        sbs_union_test_CAC = fitness_function_CAC.evaluate_on_test(sbs_union)
        sbs_union_test_F1 = fitness_function_F1.evaluate_on_test(sbs_union)
        sbs_union_db_index = get_Davies_Bouldin_score(X, y, sbs_union)
        sbs_union_silhouette_index = get_silhouette_score(X, y, sbs_union)

        sbs_union = sbs_union * 1
        results["sbs_union_solution"] = ','.join([str(feature) for feature in sbs_union])
        results["sbs_union_validation_fitness"] = sbs_union_validation_fitness
        results["sbs_union_test_CAC"] = sbs_union_test_CAC
        results["sbs_union_test_F1"] = sbs_union_test_F1
        results["sbs_union_size"] = np.sum(sbs_union)
        results["sbs_union_DB"] = sbs_union_db_index
        results["sbs_union_silhouette"] = sbs_union_silhouette_index

        test_sensitivity = fitness_function_sensitivity.evaluate_on_test(sbs_union)
        results["sbs_union_test_sensitivity"] = test_sensitivity
        results["sbs_union_kfold_fitness"] = sbs_fitness

    if aggregation_principles_to_evaluate['sfs_intersection']:
        activated_features = np.sum(population, axis=0) / len(population)
        activated_features_sorted_indices = np.argsort(activated_features)[::-1]
        for solution in population:
            intersection = intersection * np.array(solution).astype(bool)

        intersection_fitness = fitness_function_CV.evaluate_on_validation(intersection)
        sfs_intersection = copy.deepcopy(intersection)
        sfs_fitness = intersection_fitness
        for j in activated_features_sorted_indices:
            if activated_features[j] != 0 and activated_features[j] != 1:
                sfs_intersection[j] = True
                old_fitness = sfs_fitness
                sfs_fitness = fitness_function_CV.evaluate_on_validation(sfs_intersection)

                if sfs_fitness <= old_fitness:
                    sfs_intersection[j] = False
                    sfs_fitness = old_fitness
                    # break

        sfs_intersection_validation_fitness = fitness_function.evaluate_on_validation(sfs_intersection)
        sfs_intersection_test_CAC = fitness_function_CAC.evaluate_on_test(sfs_intersection)
        sfs_intersection_test_F1 = fitness_function_F1.evaluate_on_test(sfs_intersection)
        sfs_intersection_db_index = get_Davies_Bouldin_score(X, y, sfs_intersection)
        sfs_intersection_silhouette_index = get_silhouette_score(X, y, sfs_intersection)

        sfs_intersection = sfs_intersection * 1
        results["sfs_intersection_solution"] = ','.join([str(feature) for feature in sfs_intersection])
        results["sfs_intersection_validation_fitness"] = sfs_intersection_validation_fitness
        results["sfs_intersection_test_CAC"] = sfs_intersection_test_CAC
        results["sfs_intersection_test_F1"] = sfs_intersection_test_F1
        results["sfs_intersection_size"] = np.sum(sfs_intersection)
        results["sfs_intersection_DB"] = sfs_intersection_db_index
        results["sfs_intersection_silhouette"] = sfs_intersection_silhouette_index

        test_sensitivity = fitness_function_sensitivity.evaluate_on_test(sfs_intersection)
        results["sfs_intersection_test_sensitivity"] = test_sensitivity
        results["sfs_intersection_kfold_fitness"] = sfs_fitness

    if aggregation_principles_to_evaluate['sfs_sbs_majority_voting']:
        activated_features = np.sum(population, axis=0) / len(population)
        majority_voting = activated_features >= 0.5
        feature_thresholds = np.asarray([abs(share - 0.5) for share in activated_features])
        activated_features_sorted_indices = np.argsort(feature_thresholds)

        majority_voting_fitness = fitness_function_CV.evaluate_on_validation(majority_voting)
        sbs_sfs_majority_voting = copy.deepcopy(majority_voting)
        sbs_sfs_majority_voting_fitness = majority_voting_fitness
        for j in activated_features_sorted_indices:
            if feature_thresholds[j] != 0.5:
                old_fitness = sbs_sfs_majority_voting_fitness
                if activated_features[j] < 0.5:
                    sbs_sfs_majority_voting[j] = True
                    sbs_sfs_majority_voting_fitness = fitness_function_CV.evaluate_on_validation(
                        sbs_sfs_majority_voting)

                    if sbs_sfs_majority_voting_fitness <= old_fitness:
                        sbs_sfs_majority_voting[j] = False
                        sbs_sfs_majority_voting_fitness = old_fitness
                        # break

                elif activated_features[j] > 0.5:
                    sbs_sfs_majority_voting[j] = False
                    sbs_sfs_majority_voting_fitness = fitness_function_CV.evaluate_on_validation(
                        sbs_sfs_majority_voting)
        ...
        sbs_sfs_majority_voting_validation_fitness = fitness_function.evaluate_on_validation(sbs_sfs_majority_voting)
        sbs_sfs_majority_voting_test_CAC = fitness_function_CAC.evaluate_on_test(sbs_sfs_majority_voting)
        sbs_sfs_majority_voting_test_F1 = fitness_function_F1.evaluate_on_test(sbs_sfs_majority_voting)
        sbs_sfs_majority_voting_db_index = get_Davies_Bouldin_score(X, y, sbs_sfs_majority_voting)
        sbs_sfs_majority_voting_silhouette_index = get_silhouette_score(X, y, sbs_sfs_majority_voting)

        sbs_sfs_majority_voting = sbs_sfs_majority_voting * 1
        results["sbs_sfs_majority_voting"] = ','.join([str(feature) for feature in sbs_sfs_majority_voting])
        results["sbs_sfs_majority_voting_validation_fitness"] = sbs_sfs_majority_voting_validation_fitness
        results["sbs_sfs_majority_voting_test_CAC"] = sbs_sfs_majority_voting_test_CAC
        results["sbs_sfs_majority_voting_test_F1"] = sbs_sfs_majority_voting_test_F1
        results["sbs_sfs_majority_voting_size"] = np.sum(sbs_sfs_majority_voting)
        results["sbs_sfs_majority_voting_DB"] = sbs_sfs_majority_voting_db_index
        results["sbs_sfs_majority_voting_silhouette"] = sbs_sfs_majority_voting_silhouette_index

        test_sensitivity = fitness_function_sensitivity.evaluate_on_test(sbs_sfs_majority_voting)
        results["sbs_sfs_majority_voting_test_sensitivity"] = test_sensitivity
        results["sbs_sfs_majority_voting_kfold_fitness"] = sbs_sfs_majority_voting_fitness
    ...
    if aggregation_principles_to_evaluate['intersection']:
        ...
        test_sensitivity = fitness_function_sensitivity.evaluate_on_test(intersection)
        kfold_fitness = fitness_function_CV.evaluate_on_validation(intersection)
        results["intersection_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['union']:
        ...
        test_sensitivity = fitness_function_sensitivity.evaluate_on_test(union)
        kfold_fitness = fitness_function_CV.evaluate_on_validation(union)
        results["union_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['multiintersection']:
        ...
        test_sensitivity = fitness_function_sensitivity.evaluate_on_test(multiintersection)
        kfold_fitness = fitness_function_CV.evaluate_on_validation(multiintersection)
        results["multiintersection_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['majority_voting']:
        ...
        test_sensitivity = fitness_function_sensitivity.evaluate_on_test(coalition_solution_majority_voting)
        kfold_fitness = fitness_function_CV.evaluate_on_validation(coalition_solution_majority_voting)
        results["coalition_solution_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['minority_voting']:
        ...
        test_sensitivity = fitness_function_sensitivity.evaluate_on_test(coalition_solution_minority_voting)
        kfold_fitness = fitness_function_CV.evaluate_on_validation(coalition_solution_minority_voting)
        results["coalition_solution_minority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['large_majority_voting']:
        ...
        test_sensitivity = fitness_function_sensitivity.evaluate_on_test(coalition_solution_large_majority_voting)
        kfold_fitness = fitness_function_CV.evaluate_on_validation(coalition_solution_large_majority_voting)
        results["coalition_solution_large_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['fitness_weighted_majority_voting']:
        ...
        results["coalition_fitness_weighted_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['fitness_rank_weighted_majority_voting']:
        ...
        total_votes = 0
        for idx, candidate in enumerate(coalition):
            rank = coalition_fitness[coalition_fitness <= coalition_fitness[idx]].shape[0]
            activated_features += rank * np.array(candidate).astype(int)
            total_votes += rank
        ...
        results["coalition_fitness_rank_weighted_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['size_weighted_majority_voting']:
        ...
        results["coalition_size_weighted_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['fitness_size_rank_weighted_majority_voting_solutions']:
        ...
        results["coalition_fitness_size_rank_weighted_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['fitness_size_weighted_majority_voting_solutions']:
        ...
        total_votes = 0
        for idx, candidate in enumerate(coalition):
            weight = coalition_fitness[idx] + ((len(candidate) - np.sum(candidate)) / len(candidate))
            activated_features += weight * np.array(candidate).astype(int)
            total_votes += weight
        ...
        results["coalition_fitness_size_weighted_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['db_rank_weighted_majority_voting_solutions']:
        ...
        results["coalition_db_rank_weighted_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['db_weighted_majority_voting_solutions']:
        ...
        results["coalition_db_weighted_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['pairwise_feature_frequency_majority_voting_solutions']:
        ...
        results["coalition_pairwise_feature_frequency_majority_voting_test_sensitivity"] = test_sensitivity
        results["coalition_pairwise_feature_frequency_majority_voting_kfold_fitness"] = kfold_fitness

    union = np.array([False] * len(coalition[0]), dtype=bool)
    intersection = np.array([True] * len(coalition[0]), dtype=bool)
    if aggregation_principles_to_evaluate['sbs_union']:
        activated_features = np.sum(coalition, axis=0) / len(coalition)
        activated_features_sorted_indices = np.argsort(activated_features)
        for solution in coalition:
            union = union + np.array(solution).astype(bool)

        coalition_union_fitness = fitness_function_CV.evaluate_on_validation(union)
        coalition_sbs_union = copy.deepcopy(union)
        sbs_fitness = coalition_union_fitness
        for j in activated_features_sorted_indices:
            if activated_features[j] != 0 and activated_features[j] != 1:
                coalition_sbs_union[j] = False
                old_fitness = sbs_fitness
                sbs_fitness = fitness_function_CV.evaluate_on_validation(coalition_sbs_union)

                if sbs_fitness < old_fitness:
                    coalition_sbs_union[j] = True
                    sbs_fitness = old_fitness
                    # break

        coalition_sbs_union_validation_fitness = fitness_function.evaluate_on_validation(coalition_sbs_union)
        coalition_sbs_union_test_CAC = fitness_function_CAC.evaluate_on_test(coalition_sbs_union)
        coalition_sbs_union_test_F1 = fitness_function_F1.evaluate_on_test(coalition_sbs_union)
        coalition_sbs_union_db_index = get_Davies_Bouldin_score(X, y, coalition_sbs_union)
        coalition_sbs_union_silhouette_index = get_silhouette_score(X, y, coalition_sbs_union)

        coalition_sbs_union = coalition_sbs_union * 1
        results["coalition_sbs_union_solution"] = ','.join([str(feature) for feature in coalition_sbs_union])
        results["coalition_sbs_union_validation_fitness"] = coalition_sbs_union_validation_fitness
        results["coalition_sbs_union_test_CAC"] = coalition_sbs_union_test_CAC
        results["coalition_sbs_union_test_F1"] = coalition_sbs_union_test_F1
        results["coalition_sbs_union_size"] = np.sum(coalition_sbs_union)
        results["coalition_sbs_union_DB"] = coalition_sbs_union_db_index
        results["coalition_sbs_union_silhouette"] = coalition_sbs_union_silhouette_index

        test_sensitivity = fitness_function_sensitivity.evaluate_on_test(coalition_sbs_union)
        results["coalition_sbs_union_test_sensitivity"] = test_sensitivity
        results["coalition_sbs_union_kfold_fitness"] = sbs_fitness
    ...
    if aggregation_principles_to_evaluate['sfs_intersection']:
        ...
        intersection_fitness = fitness_function_CV.evaluate_on_validation(intersection)
        coalition_sfs_intersection = copy.deepcopy(intersection)
        sfs_fitness = intersection_fitness
        for j in activated_features_sorted_indices:
            if activated_features[j] != 0 and activated_features[j] != 1:
                coalition_sfs_intersection[j] = True
                old_fitness = sfs_fitness
                sfs_fitness = fitness_function_CV.evaluate_on_validation(coalition_sfs_intersection)

                if sfs_fitness <= old_fitness:
                    coalition_sfs_intersection[j] = False
                    sfs_fitness = old_fitness
                    # break

        coalition_sfs_intersection_validation_fitness = fitness_function.evaluate_on_validation(
            coalition_sfs_intersection)
        coalition_sfs_intersection_test_CAC = fitness_function_CAC.evaluate_on_test(coalition_sfs_intersection)
        coalition_sfs_intersection_test_F1 = fitness_function_F1.evaluate_on_test(coalition_sfs_intersection)
        coalition_sfs_intersection_db_index = get_Davies_Bouldin_score(X, y, coalition_sfs_intersection)
        coalition_sfs_intersection_silhouette_index = get_silhouette_score(X, y, coalition_sfs_intersection)

        coalition_sfs_intersection = coalition_sfs_intersection * 1
        results["coalition_sfs_intersection_solution"] = ','.join(
            [str(feature) for feature in coalition_sfs_intersection])
        results["coalition_sfs_intersection_validation_fitness"] = coalition_sfs_intersection_validation_fitness
        results["coalition_sfs_intersection_test_CAC"] = coalition_sfs_intersection_test_CAC
        results["coalition_sfs_intersection_test_F1"] = coalition_sfs_intersection_test_F1
        results["coalition_sfs_intersection_size"] = np.sum(coalition_sfs_intersection)
        results["coalition_sfs_intersection_DB"] = coalition_sfs_intersection_db_index
        results["coalition_sfs_intersection_silhouette"] = coalition_sfs_intersection_silhouette_index

        test_sensitivity = fitness_function_sensitivity.evaluate_on_test(coalition_sfs_intersection)
        results["coalition_sfs_intersection_test_sensitivity"] = test_sensitivity
        results["coalition_sfs_intersection_kfold_fitness"] = sfs_fitness

    if aggregation_principles_to_evaluate['sfs_sbs_majority_voting']:
        activated_features = np.sum(coalition, axis=0) / len(coalition)
        coalition_sbs_sfs_majority_voting = activated_features >= 0.5
        feature_thresholds = np.asarray([abs(share - 0.5) for share in activated_features])
        activated_features_sorted_indices = np.argsort(feature_thresholds)

        coalition_sbs_sfs_majority_voting_fitness = fitness_function_CV.evaluate_on_validation(
            coalition_sbs_sfs_majority_voting)
        coalition_sbs_sfs_majority_voting = copy.deepcopy(coalition_sbs_sfs_majority_voting)
        coalition_sbs_sfs_majority_voting_fitness = coalition_sbs_sfs_majority_voting_fitness
        for j in activated_features_sorted_indices:
            if feature_thresholds[j] != 0.5:
                old_fitness = coalition_sbs_sfs_majority_voting_fitness
                if activated_features[j] < 0.5:
                    coalition_sbs_sfs_majority_voting[j] = True
                    coalition_sbs_sfs_majority_voting_fitness = fitness_function_CV.evaluate_on_validation(
                        coalition_sbs_sfs_majority_voting)

                    if coalition_sbs_sfs_majority_voting_fitness <= old_fitness:
                        coalition_sbs_sfs_majority_voting[j] = False
                        coalition_sbs_sfs_majority_voting_fitness = old_fitness
                        # break
        ...
        coalition_sbs_sfs_majority_voting_validation_fitness = fitness_function.evaluate_on_validation(
            coalition_sbs_sfs_majority_voting)
        coalition_sbs_sfs_majority_voting_test_CAC = fitness_function_CAC.evaluate_on_test(
            coalition_sbs_sfs_majority_voting)
        coalition_sbs_sfs_majority_voting_test_F1 = fitness_function_F1.evaluate_on_test(
            coalition_sbs_sfs_majority_voting)
        coalition_sbs_sfs_majority_voting_db_index = get_Davies_Bouldin_score(X, y,
                                                                              coalition_sbs_sfs_majority_voting)
        coalition_sbs_sfs_majority_voting_silhouette_index = get_silhouette_score(X, y,
                                                                                  coalition_sbs_sfs_majority_voting)

        coalition_sbs_sfs_majority_voting = coalition_sbs_sfs_majority_voting * 1
        results["coalition_sbs_sfs_majority_voting"] = ','.join(
            [str(feature) for feature in coalition_sbs_sfs_majority_voting])
        results[
            "coalition_sbs_sfs_majority_voting_validation_fitness"] = coalition_sbs_sfs_majority_voting_validation_fitness
        results[
            "coalition_sbs_sfs_majority_voting_test_CAC"] = coalition_sbs_sfs_majority_voting_test_CAC
        results["coalition_sbs_sfs_majority_voting_test_F1"] = coalition_sbs_sfs_majority_voting_test_F1
        results["oalition_sbs_sfs_majority_voting_size"] = np.sum(coalition_sbs_sfs_majority_voting)
        results["coalition_sbs_sfs_majority_voting_DB"] = coalition_sbs_sfs_majority_voting_db_index
        results[
            "coalition_sbs_sfs_majority_voting_silhouette"] = coalition_sbs_sfs_majority_voting_silhouette_index

        test_sensitivity = fitness_function_sensitivity.evaluate_on_test(coalition_sbs_sfs_majority_voting)
        results["coalition_sbs_sfs_majority_voting_test_sensitivity"] = test_sensitivity
        results[
            "coalition_sbs_sfs_majority_voting_kfold_fitness"] = coalition_sbs_sfs_majority_voting_fitness
    ...
    if aggregation_principles_to_evaluate['union']:
        ...
        results["coalition_solution_union_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['intersection']:
        ...
        results["coalition_solution_intersection_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['multiintersection']:
        ...
        results["coalition_solution_multiintersection_test_sensitivity"] = test_sensitivity
    ...
    for idx1 in range(len(population) - 1):
        ...
        for idx2 in range(idx1 + 1, len(population)):
            if get_Hamming_distance(population[idx1], population[idx2]) == 1:
                count += 1
                if population_fitness[idx1] <= population_fitness[idx2] and sum(population[idx1]) > sum(
                        population[idx2]):
                    if idx1 not in indices_to_throw:
                        indices_to_throw.append(idx1)
                elif population_fitness[idx2] <= population_fitness[idx1] and sum(population[idx2]) > sum(
                        population[idx1]):
                    if idx2 not in indices_to_throw:
                        indices_to_throw.append(idx2)
        similar_counts.append(count)
    ...
    if aggregation_principles_to_evaluate['majority_voting']:
        ...
        test_sensitivity = fitness_function_sensitivity.evaluate_on_test(reduced_solution_majority_voting)
        kfold_fitness = fitness_function_CV.evaluate_on_validation(reduced_solution_majority_voting)
        results["reduced_solution_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['minority_voting']:
        ...
        test_sensitivity = fitness_function_sensitivity.evaluate_on_test(reduced_solution_minority_voting)
        kfold_fitness = fitness_function_CV.evaluate_on_validation(reduced_solution_minority_voting)
        results["reduced_solution_minority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['large_majority_voting']:
        ...
        test_sensitivity = fitness_function_sensitivity.evaluate_on_test(reduced_solution_large_majority_voting)
        kfold_fitness = fitness_function_CV.evaluate_on_validation(reduced_solution_large_majority_voting)
        results["reduced_solution_large_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['fitness_weighted_majority_voting']:
        ...
        results["reduced_fitness_weighted_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['fitness_rank_weighted_majority_voting']:
        ...
        results["reduced_fitness_rank_weighted_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['size_weighted_majority_voting']:
        ...
        results["reduced_size_weighted_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['fitness_size_rank_weighted_majority_voting_solutions']:
        ...
        results["reduced_fitness_size_rank_weighted_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['fitness_size_weighted_majority_voting_solutions']:
        ...
        results["reduced_fitness_size_weighted_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['db_rank_weighted_majority_voting_solutions']:
        ...
        results["reduced_db_rank_weighted_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['db_weighted_majority_voting_solutions']:
        ...
        results["reduced_db_weighted_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['pairwise_feature_frequency_majority_voting_solutions']:
        ...
        results["reduced_pairwise_feature_frequency_majority_voting_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['union']:
        ...
        results["reduced_solution_union_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['intersection']:
        ...
        results["reduced_solution_intersection_test_sensitivity"] = test_sensitivity
    ...
    if aggregation_principles_to_evaluate['multiintersection']:
        ...
        results["reduced_solution_multiintersection_test_sensitivity"] = test_sensitivity
...
def get_stability_report(summary_results, dataset, stability_function):
    ...
    sbs_union_solutions = []
    sfs_intersection_solutions = []
    sbs_sfs_majority_voting_solutions = []
    ...
    coalition_sbs_union_solutions = []
    coalition_sfs_intersection_solutions = []
    coalition_sbs_sfs_majority_voting_solutions = []
    ...
    for run_summary in summary_results:
        if run_summary["dataset"] == dataset:
            ...
            if aggregation_principles_to_evaluate['sbs_union']:
                sbs_union_solutions.append(
                    np.array(
                        [True if feature == '1' else False for feature in
                         run_summary["sbs_union_solution"].split(',')]).astype(
                        bool))
                coalition_sbs_union_solutions.append(
                    np.array([True if feature == '1' else False for feature in
                              run_summary["coalition_sbs_union_solution"].split(',')]).astype(bool))

                stability_report["sbs_union_solution"] = stability_function(sbs_union_solutions)
                stability_report["coalition_sbs_union_solution"] = stability_function(coalition_sbs_union_solutions)
            if aggregation_principles_to_evaluate['sfs_intersection']:
                sfs_intersection_solutions.append(
                    np.array(
                        [True if feature == '1' else False for feature in
                         run_summary["sfs_intersection_solution"].split(',')]).astype(
                        bool))
                coalition_sfs_intersection_solutions.append(
                    np.array([True if feature == '1' else False for feature in
                              run_summary["coalition_sfs_intersection_solution"].split(',')]).astype(bool))

                stability_report["sfs_intersection_solution"] = stability_function(sfs_intersection_solutions)
                stability_report["coalition_sfs_intersection_solution"] = stability_function(
                    coalition_sfs_intersection_solutions)

            if aggregation_principles_to_evaluate['sfs_sbs_majority_voting']:
                sbs_sfs_majority_voting_solutions.append(
                    np.array(
                        [True if feature == '1' else False for feature in
                         run_summary["sbs_sfs_majority_voting"].split(',')]).astype(
                        bool))
                coalition_sbs_sfs_majority_voting_solutions.append(
                    np.array([True if feature == '1' else False for feature in
                              run_summary["coalition_sbs_sfs_majority_voting"].split(',')]).astype(bool))

                stability_report["sbs_sfs_majority_voting_solution"] = stability_function(
                    sbs_sfs_majority_voting_solutions)
                stability_report["coalition_sbs_sfs_majority_voting_solution"] = stability_function(
                    coalition_sbs_sfs_majority_voting_solutions)
    ...
    for procedure in evaluation_procedures:
        for classifier in classifiers:
            ...
            for file in datasets:
                ...
                test_header, test_data = CsvProcessor().read_file(
                    filename='logs/archive/' + filename + "_test_9000")
                ...
                if test_header is not None and test_data is not None:
                    ...
                    for idx, candidate in enumerate(test_data):
                        ...
                        population_clustering_index[index] = 0
                        ...
                        population_test_fitness[index] = float(candidate[header_idx[fitness + "_test"]])
            ...
            for row in summary_results[optimiser.name + procedure + classifier.name]:
                CsvProcessor().save_summary_results(filename='_'.join(
                    [procedure, optimiser.name, classifier.name, "_archive_combinations_9000"]),
                    header=list(row.keys()),
                    data=list(row.values()))

            for row in asm_stability_results[optimiser.name + procedure + classifier.name]:
                CsvProcessor().save_summary_results(filename='_'.join(
                    [procedure, optimiser.name, classifier.name, "_archive_asm_stabilities_9000"]),
                    header=list(row.keys()),
                    data=list(row.values()))
