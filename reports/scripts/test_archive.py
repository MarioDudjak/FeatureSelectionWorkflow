    ...
    for optimiser in fs_wrappers:
        for classifier in classifiers:
            for dataset in datasets:
                ...
                filename = '_'.join(
                    ['', optimiser.name + "_", classifier.name, dataset.name,
                     'archive'])
                ...
                if archive_header is not None and archive_data is not None:
                    ...
                    for idx, candidate in enumerate(candidates):
                        if idx > 0 and idx % evaluations == 0:
                            ...
                            for j, archive_el in enumerate(archive):
                                ...
                                CsvProcessor().save_log_results(
                                    filename='archive/' + filename + "_test",
                                    header=metrics,
                                    data=results)
                        ...
                        index += 1
                        if (not any(np.array_equal(archive_element, feature_vector) for archive_element in
                                    archive)) and (validation_fitness > np.min(archive_fitness) or (
                                validation_fitness == np.min(archive_fitness) and np.sum(feature_vector) < np.sum(
