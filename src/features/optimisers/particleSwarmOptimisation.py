...
class ParticleSwarmOptimisation(Wrapper):
    ...
    def search(self, experiment_name, fitness_function, initial_population=None):
        ...
        for i in range(self.population_size):
            ...
            archive[i] = copy.deepcopy(binary_population[i])
            archive_fitness[i] = particle_fitness[i]
        ...
        while spent_nfes < self.max_nfes:
            for i in range(self.population_size):
                ...
                if (archive_duration == 0 and not any(np.array_equal(archive_element, binary_trial_population[i]) for archive_element in archive)) and (particle_fitness[i] > np.min(archive_fitness) or (particle_fitness[i] == np.min(archive_fitness) and np.sum(binary_trial_population[i]) < np.sum(archive[np.argmin(archive_fitness)]))):
                    worst_idx = np.argmin(archive_fitness)
                    archive[worst_idx] = copy.deepcopy(binary_trial_population[i])
                    archive_fitness[worst_idx] = particle_fitness[i]
        ...
        logger.log_output(output_quality, archive=copy.deepcopy(archive),
                          ...
                          population_fitness=copy.deepcopy(particle_fitness), wrapper_duration=wrapper_duration, archive_duration=archive_duration)
...
class ParticleSwarmOptimisationIniPG(Wrapper):
    """
    PSO(4-2) from the paper: Bing Xue et al., "Particle swarm optimisation for feature selection in classification: Novel initialisation and updating mechanisms"
    """
    def __init__(self, population_size, max_nfes, c1, c2, inertia, bound_handler, binarizer):
        super().__init__(population_size)
        self.max_nfes = max_nfes
