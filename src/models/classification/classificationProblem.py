...
class ClassificationProblem:
    ...
    def __init__(self, file, classifier, random_state1, random_state2, test_size, validation_size, wrapper_fitness_metric, metrics):
...
class ClassificationProblemCV:
    ...
    def __init__(self, file, classifier, random_state1, random_state2, test_size, k_folds, wrapper_fitness_metric, metrics):
        self.dataset = pd.read_csv(file.path, header=0, index_col=0)
        self.data = self.dataset.iloc[:, :-1].values
        self.target = self.dataset.iloc[:, -1]
        self.name = '_'.join([classifier.name, file.name])
        ...
        self.cv = StratifiedKFold(n_splits=k_folds, shuffle=True, random_state=random_state2)
