...
class EGAFS(Wrapper):
    ...
    """
    def __init__(self, population_size, max_nfes, crossover_rate, mutation_rate, balancing_factor, crossover_operator, mutation_operator,
                 parent_selection_operator, population_selection_operator):
        super().__init__(population_size)
        self.max_nfes = max_nfes
        self.crossover_rate = crossover_rate
    ...
    def search(self, experiment_name, fitness_function, initial_population=None):
        ...
        for index, candidate in enumerate(population):
            if not any(candidate):
                ...
                population_fitness[index] = self.balancing_factor * fitness_function.evaluate_on_validation(population[index],
        ...
        while spent_nfes < self.max_nfes:
            ...
            for index, candidate in enumerate(new_population):
                if not any(candidate):
                    ...
                    new_population_fitness[index] = self.balancing_factor * fitness_function.evaluate_on_validation(new_population[index],
