...
fs_wrappers = [
    ...
    ParticleSwarmOptimisation(population_size=30, max_nfes=global_params["evaluations"], c1=1.496, c2=1.496, inertia=0.7298, bound_handler=LimitBoundHandler(0,1), binarizer=FixedBinariser()),
    # ParticleSwarmOptimisationIniPG(population_size=30, max_nfes=3000, c1=1.49618, c2=1.49618,
    #                                inertia=0.7298, bound_handler=LimitBoundHandler(0, 1), binarizer=FixedBinariser())
GeneticAlgorithm(global_params["populationSize"], global_params["evaluations"], 0.9, crossovers.SinglePointCrossover(), mutations.BitFlipMutation(), parent_selections.Tournament(k=2), population_selections.MuPlusLambda()),
    DifferentialEvolution(global_params["populationSize"], global_params["evaluations"], 0.9, 0.5, LimitBoundHandler(0,1), FixedBinariser())
]
